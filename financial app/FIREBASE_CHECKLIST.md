...
The current app code uses Firebase Authentication in `mobile/lib/screens/auth_screen.dart` for sign-up and login. It also saves user profile and transaction data to Firestore.

The backend can optionally verify Firebase ID tokens when `FIREBASE_AUTH_REQUIRED=true` using `backend/app/core/auth.py`. In that mode, requests must include:
